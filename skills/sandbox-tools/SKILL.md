...
必ずDockerコンテナ（sandbox-tools）で実行すること。

プロジェクト自体のコード（テスト、lint、アプリ起動など）はプロジェクトの環境で実行する。

`/sandbox-tools` に引数が渡された場合: $ARGUMENTS
...
コマンドを実行する:
```bash
${CLAUDE_SKILL_DIR}/scripts/sandbox-tools python3 script.py arg1 arg2
${CLAUDE_SKILL_DIR}/scripts/sandbox-tools pdftotext input.pdf output.txt
${CLAUDE_SKILL_DIR}/scripts/sandbox-tools ffmpeg -i input.mp4 output.gif
```

標準入力でPythonコードを渡す:
```bash
${CLAUDE_SKILL_DIR}/scripts/sandbox-tools python3 - <<'PYTHON'
print("Hello")
PYTHON
```
...
```bash
${CLAUDE_SKILL_DIR}/scripts/sandbox-tools --commit bash -c "apt-get update && apt-get install -y パッケージ名 && rm -rf /var/lib/apt/lists/*"
${CLAUDE_SKILL_DIR}/scripts/sandbox-tools --commit pip install パッケージ名
```
...
```bash
${CLAUDE_SKILL_DIR}/scripts/sandbox-tools --dir /path/to/input --dir /path/to/output <<'PYTHON'
import pandas as pd
